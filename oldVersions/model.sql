--- Model de dades de CtaDom (PostgreSQL) :
----------------------------------------------


CREATE TABLE comptes_reals
(
  codi_compte_real text NOT NULL,
  descripcio text NOT NULL,
  import_actual numeric(15,2),
  id_ultim_moviment integer,
  CONSTRAINT comptes_reals_pk PRIMARY KEY (codi_compte_real)
) WITH (OIDS=FALSE);
ALTER TABLE comptes_reals OWNER TO barba;

  
insert into comptes_reals
select '1. Cta. Vell', 			'Cta. Vell', 		0, 0 union all
select '2. Cta. Nou', 			'Cta. Nou', 		0, 0 union all
select '3. Efectiu caixa', 		'Efectiu caixa', 	0, 0 union all
select '4. Efectiu cartera', 	'Efectiu cartera', 	0, 0 union all
select '5. Cta. BS', 			'Cta. BS', 			0, 0 union all
select '6. Termini BS', 		'Termini BS', 		0, 0 union all
select '7. Traspas C', 			'Traspas C', 		0, 0;

update comptes_reals set id_ultim_moviment = null;
...
(
  ...
  codi_compte_comptable_pare 	text,
  descripcio 					text			NOT NULL,
  import_actual 				numeric(15,2),
  id_ultim_moviment 			integer,
  CONSTRAINT comptes_comptables_pk PRIMARY KEY (codi_compte_comptable)
) WITH (OIDS=FALSE);
ALTER TABLE comptes_comptables OWNER TO barba;


insert into comptes_comptables
select '1. General',		'x',	'General',		0, 0 union all
select '2. Ingressos',		'x',	'Ingressos',	0, 0 union all
select '3. Prestecs',		'x',	'Prestecs',		0, 0 union all
select '4. Vivenda',		'x',	'Vivenda',		0, 0 union all
select '5. Serveis',		'x',	'Serveis',		0, 0 union all
select '6. Cotxe',			'x',	'Cotxe',		0, 0 union all
select '7. Material',		'x',	'Material',		0, 0 union all
select '8. Menjar',			'x',	'Menjar',		0, 0 union all
select '9. Gastos varis',	'x',	'Gastos varis',	0, 0;
